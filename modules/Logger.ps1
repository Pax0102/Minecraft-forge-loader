function Write-Log
{
    ...
    # config.json "logs": false desativa a gravacao em update.log. Antes
    # essa chave existia no config.json mas nada no codigo lia ela.
    if($Config -and $Config.logs -eq $false){
        return
    }
